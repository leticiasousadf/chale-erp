...
export function DiarioClient({ fotos, etapas }: any) {
  ...
  async function handleSalvar() {
    ...
    try {
      if (arquivoSelecionado) {
        foto_url = await uploadArquivo(arquivoSelecionado, 'fotos')
      }
      const payload = {
        data: form.data,
  ...
  const isImagem = (url: string) => url && url.startsWith('http') && (url.includes('.jpg') || url.includes('.jpeg') || url.includes('.png') || url.includes('.webp'))
        ...
        {fotos.map((f: any) => (
          <div key={f.id} className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/50 transition-colors">
            ...
            <div className="p-3">
              ...
              {f.responsavel && <div className="text-xs text-muted-foreground mt-1">👤 {f.responsavel}</div>}
              <div className="flex gap-2 mt-2">
                <Button variant="ghost" size="sm" className="flex-1 text-xs" onClick={() => openEdit(f)}>✏️ Editar</Button>
                <Button variant="ghost" size="sm" className="text-xs text-red-500" onClick={() => handleExcluir(f.id)}>🗑️</Button>
              </div>
            </div>
          </div>
        ))}
        <div
          className="bg-card border border-dashed border-border rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer opacity-50 hover:opacity-80 transition-opacity min-h-[160px]"
          onClick={openNew}
        >
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end md:items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setModalOpen(false)}>
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* Upload de foto */}
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Foto</label>
                <div
                  className="mt-1 border-2 border-dashed border-border rounded-xl overflow-hidden cursor-pointer hover:border-primary transition-colors"
                ...
                >
                  {preview
                    ...
                    : <div className="w-full aspect-video flex flex-col items-center justify-center gap-2 text-muted-foreground">
                        <span className="text-3xl">📷</span>
                        <span className="text-xs">Clique para selecionar foto</span>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <Button onClick={handleSalvar} disabled={isPending || uploading}>
                {uploading ? '⏳ Enviando foto...' : isPending ? 'Salvando...' : 'Salvar registro'}
              </Button>
            </div>
          </div>
        </div>
