...
async function uploadDoc(file: File): Promise<string> {
  ...
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/documentos/${nome}`, {
    method: 'POST',
    headers: {
      'apikey': SUPABASE_KEY,
  ...
  return `${SUPABASE_URL}/storage/v1/object/public/documentos/${nome}`
}
...
const TIPO_EMOJI: Record<string, string> = { PDF: '📄', PNG: '🖼️', JPG: '🖼️', JPEG: '🖼️', XLSX: '📊', XLS: '📊', DOC: '📝', DOCX: '📝' }
...
export function DocumentosClient({ documentos }: any) {
  ...
  function openNew() {
    setEditItem(null); setArquivo(null)
    setForm({ nome: '', pasta: 'Contratos', tipo: 'PDF', emoji: '📄', arquivo_url: '' })
    setModalOpen(true)
  }

  function openEdit(d: any) {
    setEditItem(d); setArquivo(null)
  ...
  async function handleSalvar() {
    ...
    try {
      if (arquivo) arquivo_url = await uploadDoc(arquivo)
      const payload = { nome: form.nome, pasta: form.pasta, tipo: form.tipo, emoji: form.emoji, arquivo_url: arquivo_url || 'placeholder' }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {pastas.map(p => (
          <button key={p} onClick={() => setFiltro(p)} className={`px-3 py-1 rounded-full text-xs font-semibold border transition-colors ${filtro === p ? 'border-primary text-primary bg-primary/10' : 'border-border text-muted-foreground bg-card'}`}>
            {p || 'Todos'}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          return (
            <div key={d.id} className="bg-card border border-border rounded-xl p-4 flex flex-col gap-2 hover:border-primary/50 transition-colors">
              ...
              <div className="flex gap-1 mt-1 flex-wrap">
                {temArquivo && (
                  <a href={d.arquivo_url} target="_blank" rel="noopener noreferrer" className="flex-1">
                    <Button variant="ghost" size="sm" className="text-xs w-full">⬇️ Baixar</Button>
                  </a>
                )}
        ...
        <div className="bg-card border border-dashed border-border rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer opacity-50 hover:opacity-80 min-h-[140px] transition-opacity" onClick={openNew}>
          <span className="text-3xl">📎</span><span className="text-xs text-muted-foreground">Novo documento</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end md:items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setModalOpen(false)}>
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide block mb-1">Arquivo</label>
                <label className="flex items-center gap-3 w-full py-4 px-4 rounded-xl border-2 border-dashed border-border cursor-pointer hover:border-primary hover:bg-primary/5 transition-colors">
                  <span className="text-2xl">{arquivo ? '✅' : '📎'}</span>
                  <div>
                    <div className="text-sm font-medium">{arquivo ? arquivo.name : 'Selecionar arquivo'}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{arquivo ? 'Clique para trocar' : 'PDF, PNG, JPG, XLSX, DOCX'}</div>
                  </div>
                  <input type="file" accept=".pdf,.png,.jpg,.jpeg,.xlsx,.xls,.doc,.docx" className="hidden" onChange={handleFile} />
                </label>
              </div>
