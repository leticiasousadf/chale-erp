...
export function TimelineClient({etapas}:any){
  ...
  function openNew(){setEi(null);setForm({nome:'',previsao_inicio:'',previsao_fim:'',status:'planejado',percentual:'0',descricao:''});setMo(true)}
  function openEdit(e:any){setEi(e);setForm({nome:e.nome,previsao_inicio:e.previsao_inicio||'',previsao_fim:e.previsao_fim||'',status:e.status,percentual:String(e.percentual||0),descricao:e.descricao||''});setMo(true)}
  function salvar(){st(async()=>{const p={...form,percentual:parseInt(form.percentual)||0};if(ei)await updateEtapa(ei.id,p);else await createEtapa(p);setMo(false);router.refresh()})}
  function excluir(id:string){if(!confirm('Excluir?'))return;st(async()=>{await deleteEtapa(id);router.refresh()})}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col">
        {etapas.map((e:any,i:number)=>(
          <div key={e.id} className="grid" style={{gridTemplateColumns:'130px 1px 1fr',columnGap:'20px'}}>
            <div className="text-right pb-8 pt-0.5"><div className="text-xs font-semibold text-muted-foreground">{fd(e.previsao_inicio)}</div><div className="text-xs text-muted-foreground opacity-60 mt-1">{LABEL[e.status]}</div></div>
              ...
              <div className="bg-card border rounded-xl p-4" style={{borderColor:e.status==='em_andamento'?COR[e.status]:''}}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs border border-border rounded-full px-2 py-0.5">{LABEL[e.status]}</span>
                    <Button variant="ghost" size="sm" onClick={()=>openEdit(e)}>✏️</Button>
                    <Button variant="ghost" size="sm" onClick={()=>excluir(e.id)}>🗑️</Button>
                  </div>
                </div>
      ...
      {mo&&(<div className="fixed inset-0 bg-black/70 z-50 flex items-end md:items-center justify-center p-4" onClick={e=>e.target===e.currentTarget&&setMo(false)}>
        <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <h3 className="text-lg font-bold mb-5">{ei?'Editar':'Nova'} Etapa</h3>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-xs font-semibold text-muted-foreground uppercase">Início previsto</label><input type="date" className="w-full mt-1 px-3 py-2 text-sm rounded-lg border border-border bg-muted text-foreground outline-none focus:border-primary" value={form.previsao_inicio} onChange={e=>fi('previsao_inicio',e.target.value)}/></div>
            <div><label className="text-xs font-semibold text-muted-foreground uppercase">Fim previsto</label><input type="date" className="w-full mt-1 px-3 py-2 text-sm rounded-lg border border-border bg-muted text-foreground outline-none focus:border-primary" value={form.previsao_fim} onChange={e=>fi('previsao_fim',e.target.value)}/></div>
            <div><label className="text-xs font-semibold text-muted-foreground uppercase">Status</label><select className="w-full mt-1 px-3 py-2 text-sm rounded-lg border border-border bg-muted text-foreground outline-none focus:border-primary" value={form.status} onChange={e=>fi('status',e.target.value)}><option value="planejado">Planejado</option><option value="em_andamento">Em andamento</option><option value="concluido">Concluído</option></select></div>
            <div><label className="text-xs font-semibold text-muted-foreground uppercase">Progresso (%)</label><input type="number" min="0" max="100" className="w-full mt-1 px-3 py-2 text-sm rounded-lg border border-border bg-muted text-foreground outline-none focus:border-primary" value={form.percentual} onChange={e=>fi('percentual',e.target.value)}/></div>
            <div className="col-span-2"><label className="text-xs font-semibold text-muted-foreground uppercase">Descrição</label><textarea className="w-full mt-1 px-3 py-2 text-sm rounded-lg border border-border bg-muted text-foreground outline-none focus:border-primary resize-none" rows={2} value={form.descricao} onChange={e=>fi('descricao',e.target.value)}/></div>
          </div>
