...
export default async function Dashboard() {
  ...
  const [{ data: todos }, { data: mes }, { data: pend }, { data: orc }, { data: ult }] = await Promise.all([
    ...
    supabase.from('gastos').select('valor').gte('data', im).lte('data', fm),
    supabase.from('gastos').select('valor').eq('status', 'pendente'),
    supabase.from('orcamentos').select('valor_previsto'),
    supabase.from('gastos').select('*, categorias(nome,cor)').order('created_at', { ascending: false }).limit(5),
  ])
  ...
  const fmtDate = (d: string) => {
    if (!d) return '—'
    const p = d.split('-')
    return `${p[2]}/${['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'][+p[1] - 1]}`
  }

  const statusCls: Record<string, string> = {
    pago: 'bg-green-500/15 text-green-500',
    pendente: 'bg-amber-500/15 text-amber-500',
    parcelado: 'bg-blue-500/15 text-blue-500',
  }
    ...
    <div className="space-y-6">
        ...
        {cards.map(c => (
          <Card key={c.t}>
            <CardContent className="pt-5">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">{c.t}</p>
              <p className={`text-xl font-bold font-mono mt-2 ${c.c}`}>{c.v}</p>
      ...
      <Card>
        <div className="p-4 border-b border-border flex items-center justify-between">
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Últimos Gastos</span>
          <a href="/gastos" className="text-xs text-primary">Ver todos →</a>
        </div>
          ...
          {(ult as any[] ?? []).map((g: any) => (
            <div key={g.id} className="flex items-center gap-3 p-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{g.titulo}</p>
                <p className="text-xs text-muted-foreground">{g.categorias?.nome || '—'} · {fmtDate(g.data)}</p>
              </div>
